...
import type { IStatusHistoryRepository, IConnectionRepository } from '../services/health-monitor-service';
  ...
  private async handleGetTimeseries(
    ...
    if (!since || !until) {
      return {
        statusCode: 400,
        response: {
          message: 'since and until query parameters are required',
          error: 'Bad Request',
        } as unknown as AnalyticsResponse,
      };
    }
  ...
  private async handleGetCompare(
    ...
    if (!connectionIdsParam) {
      return {
        statusCode: 400,
        response: {
          message: 'connectionIds query parameter is required',
          error: 'Bad Request',
        } as unknown as AnalyticsResponse,
      };
    }
    ...
    if (idStrings.length > 5) {
      return {
        statusCode: 400,
        response: {
          message: 'Maximum 5 connections for comparison',
          error: 'Bad Request',
        } as unknown as AnalyticsResponse,
      };
    }
    ...
    for (const idStr of idStrings) {
      ...
      if (!connId) {
        return {
          statusCode: 400,
          response: {
            message: `Invalid connection ID: ${idStr}`,
            error: 'Bad Request',
          } as unknown as AnalyticsResponse,
        };
      }
      ...
      const connection =
        await this.deps.connectionRepo.getConnection(connId);
      if (!connection) {
        return {
          statusCode: 404,
          response: {
            message: `Connection not found: ${idStr}`,
            error: 'Not Found',
          } as unknown as AnalyticsResponse,
        };
      }
    ...
    for (const connId of connectionIds) {
      const entries = await this.deps.statusHistoryRepo.getEntriesByConnection(connId, {
        since,
        until,
        signalTypes,
      });
      ...
      datasets.push({
        ...
        connectionName:
          connection?.providerUsername ?? String(connId),
        buckets,
      });
    }
