...
class MockStatusHistoryRepo implements IStatusHistoryRepository<string> {
  ...
  async getEntriesByConnection(
    ...
    _options?: { since?: Date; until?: Date; signalTypes?: HeartbeatSignalType[] },
...
function createTestSetup() {
  ...
  const application = {
    authenticationMiddleware: (_req: unknown, _res: unknown, next: () => void) => next(),
    cryptoAuthenticationMiddleware: (_req: unknown, _res: unknown, next: () => void) => next(),
...
describe('AnalyticsController', () => {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  function callHandler(controller: any, methodName: string, req: Record<string, unknown>) {
  ...
  describe('403 for non-owner', () => {
    it('should return 403 for timeseries when user does not own connection', async () => {
      ...
      connectionRepo.addConnection(makeConnection({ id: 'conn-1', userId: 'other-user' }));
    ...
    it('should return 403 for compare when user does not own one of the connections', async () => {
      const { controller, connectionRepo } = createTestSetup();
      connectionRepo.addConnection(makeConnection({ id: 'conn-1', userId: 'user-1' }));
      connectionRepo.addConnection(makeConnection({ id: 'conn-2', userId: 'other-user' }));
  ...
  describe('400 for invalid params', () => {
    it('should return 400 for timeseries with since > until', async () => {
      ...
      expect(result.response).toEqual(
        expect.objectContaining({ message: expect.stringContaining('since must be before until') }),
      );
    });
    ...
    it('should return 400 for timeseries when since/until are missing', async () => {
      ...
      expect(result.statusCode).toBe(400);
      expect(result.response).toEqual(
        expect.objectContaining({ message: expect.stringContaining('since and until') }),
      );
    });
    ...
    it('should return 400 for compare with >5 connections', async () => {
      const { controller, connectionRepo } = createTestSetup();
      for (let i = 1; i <= 6; i++) {
        connectionRepo.addConnection(makeConnection({ id: `conn-${i}`, userId: 'user-1' }));
      }
      ...
      expect(result.statusCode).toBe(400);
      expect(result.response).toEqual(
        expect.objectContaining({ message: expect.stringContaining('Maximum 5') }),
      );
    });
    ...
    it('should return 400 for compare without connectionIds', async () => {
      ...
      expect(result.statusCode).toBe(400);
      expect(result.response).toEqual(
        expect.objectContaining({ message: expect.stringContaining('connectionIds') }),
      );
    });
    ...
    it('should return 400 for export with invalid format', async () => {
      ...
      expect(result.statusCode).toBe(400);
      expect(result.response).toEqual(
        expect.objectContaining({ message: expect.stringContaining("'csv' or 'json'") }),
      );
    });
    ...
    it('should return 400 for invalid connection ID', async () => {
      ...
      expect(result.statusCode).toBe(400);
      expect(result.response).toEqual(
        expect.objectContaining({ message: expect.stringContaining('Invalid connection ID') }),
      );
    });
  });
  ...
  describe('200 with correct response shape', () => {
    ...
    function setupWithEntries() {
      const { controller, statusHistoryRepo, connectionRepo } = createTestSetup();
      connectionRepo.addConnection(makeConnection());
    ...
    it('should return 200 with IComparisonDataset[] for compare', async () => {
      const { controller, connectionRepo, statusHistoryRepo } = createTestSetup();
      connectionRepo.addConnection(makeConnection({ id: 'conn-1', userId: 'user-1' }));
      connectionRepo.addConnection(
        makeConnection({ id: 'conn-2', userId: 'user-1', providerUsername: 'user2' }),
      );
      statusHistoryRepo.setEntries([
  ...
  describe('export headers', () => {
    ...
    it('should return CSV export with correct content type and disposition', async () => {
      ...
      expect(response._contentDisposition).toMatch(/attachment; filename="history-conn-1\.csv"/);
    ...
    it('should return JSON export with correct content type and disposition', async () => {
      ...
      expect(response._contentDisposition).toMatch(/attachment; filename="history-conn-1\.json"/);
      expect(typeof response._exportData).toBe('string');
