...
describe('AnalyticsEngine', () => {
  ...
  describe('aggregateIntoBuckets', () => {
    ...
    it('should produce correct number of buckets for 6h range at 1h granularity', () => {
      const buckets = AnalyticsEngine.aggregateIntoBuckets([], since, until, '1h');
    ...
    it('should assign entries to the correct bucket', () => {
      const entries = [
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T00:30:00Z')),
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T00:45:00Z')),
        makeEntry(HeartbeatSignalType.ABSENCE, new Date('2024-01-01T02:15:00Z')),
      ];
      const buckets = AnalyticsEngine.aggregateIntoBuckets(entries, since, until, '1h');
    ...
    it('should preserve total count across all buckets', () => {
      const entries = [
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T00:30:00Z')),
        makeEntry(HeartbeatSignalType.ABSENCE, new Date('2024-01-01T01:30:00Z')),
        makeEntry(HeartbeatSignalType.CHECK_FAILED, new Date('2024-01-01T03:30:00Z')),
    ...
    it('should compute averageConfidence for non-empty buckets', () => {
      const entries = [
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T00:10:00Z'), {
          confidence: 0.8,
        }),
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T00:20:00Z'), {
          confidence: 1.0,
        }),
      ];
      const buckets = AnalyticsEngine.aggregateIntoBuckets(entries, since, until, '1h');
    ...
    it('should set dominantSignalType per bucket', () => {
      const entries = [
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T00:10:00Z')),
        makeEntry(HeartbeatSignalType.ABSENCE, new Date('2024-01-01T00:20:00Z')),
        makeEntry(HeartbeatSignalType.ABSENCE, new Date('2024-01-01T00:30:00Z')),
      ];
      const buckets = AnalyticsEngine.aggregateIntoBuckets(entries, since, until, '1h');
    ...
    it('should exclude entries outside the date range', () => {
      const entries = [
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2023-12-31T23:00:00Z')),
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T00:30:00Z')),
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T06:30:00Z')),
      ];
      const buckets = AnalyticsEngine.aggregateIntoBuckets(entries, since, until, '1h');
      const totalCount = buckets.reduce((sum, b) => sum + b.totalCount, 0);
  ...
  describe('computeStatistics', () => {
    ...
    it('should compute uptimePercentage correctly', () => {
      const entries = [
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T01:00:00Z')),
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T02:00:00Z')),
        makeEntry(HeartbeatSignalType.ABSENCE, new Date('2024-01-01T03:00:00Z')),
        makeEntry(HeartbeatSignalType.CHECK_FAILED, new Date('2024-01-01T04:00:00Z')),
      ];
    ...
    it('should compute 100% uptime when all PRESENCE', () => {
      const entries = [
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T01:00:00Z')),
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T02:00:00Z')),
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T03:00:00Z')),
      ];
      const stats = AnalyticsEngine.computeStatistics(entries, since, until);
    ...
    it('should return null averageResponseTimeMs when no PRESENCE entries have non-null values', () => {
      const entries = [
        ...
        makeEntry(HeartbeatSignalType.ABSENCE, new Date('2024-01-01T02:00:00Z'), {
    ...
    it('should compute failureRate correctly', () => {
      const entries = [
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T01:00:00Z')),
        makeEntry(HeartbeatSignalType.CHECK_FAILED, new Date('2024-01-01T02:00:00Z')),
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T03:00:00Z')),
        makeEntry(HeartbeatSignalType.CHECK_FAILED, new Date('2024-01-01T04:00:00Z')),
      ];
      const stats = AnalyticsEngine.computeStatistics(entries, since, until);
    ...
    it('should compute failureRateTrend correctly', () => {
      ...
      const entries = [
        makeEntry(HeartbeatSignalType.CHECK_FAILED, new Date('2024-01-01T06:00:00Z')),
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T08:00:00Z')),
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T14:00:00Z')),
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T18:00:00Z')),
      ];
      const stats = AnalyticsEngine.computeStatistics(entries, since, until);
    ...
    it('should return null failureRateTrend when first half has zero failure rate', () => {
      const entries = [
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T06:00:00Z')),
        makeEntry(HeartbeatSignalType.CHECK_FAILED, new Date('2024-01-01T18:00:00Z')),
      ];
      const stats = AnalyticsEngine.computeStatistics(entries, since, until);
      expect(stats.failureRateTrend).toBeNull();
    });
    ...
    it('should compute signalDistribution correctly', () => {
      ...
      expect(stats.signalDistribution[HeartbeatSignalType.CHECK_FAILED]).toBe(0);
      expect(stats.totalCheckCount).toBe(4);
    });

    it('should handle single entry', () => {
      const entries = [
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T12:00:00Z'), {
          timeSinceLastActivityMs: 5000,
        }),
      ];
      const stats = AnalyticsEngine.computeStatistics(entries, since, until);
  ...
  describe('computeHeatmap', () => {
    ...
    it('should assign entries to the correct day', () => {
      ...
      const until = new Date('2024-01-03T23:59:59Z');
      const entries = [
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T10:00:00Z')),
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T14:00:00Z')),
        makeEntry(HeartbeatSignalType.ABSENCE, new Date('2024-01-02T08:00:00Z')),
        makeEntry(HeartbeatSignalType.CHECK_FAILED, new Date('2024-01-03T20:00:00Z')),
      ];
      ...
      expect(heatmap[2].dominantSignalType).toBe(HeartbeatSignalType.CHECK_FAILED);
    });
  ...
  describe('computeStreakInfo', () => {
    ...
    it('should compute current streak from the latest entry backwards', () => {
      const entries = [
        ...
        makeEntry(HeartbeatSignalType.ABSENCE, new Date('2024-01-01T02:00:00Z')),
        makeEntry(HeartbeatSignalType.ABSENCE, new Date('2024-01-01T03:00:00Z')),
        makeEntry(HeartbeatSignalType.ABSENCE, new Date('2024-01-01T04:00:00Z')),
      ];
    ...
    it('should compute streak of 1 for single entry', () => {
      const entries = [
        makeEntry(HeartbeatSignalType.CHECK_FAILED, new Date('2024-01-01T12:00:00Z')),
      ];
      const info = AnalyticsEngine.computeStreakInfo(entries, now);
      expect(info.currentStreakCount).toBe(1);
      expect(info.currentStreakSignalType).toBe(HeartbeatSignalType.CHECK_FAILED);
    });

    it('should compute longestAbsenceDurationMs from contiguous ABSENCE runs', () => {
      const entries = [
        makeEntry(HeartbeatSignalType.ABSENCE, new Date('2024-01-01T01:00:00Z')),
        makeEntry(HeartbeatSignalType.ABSENCE, new Date('2024-01-01T03:00:00Z')),
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T04:00:00Z')),
        makeEntry(HeartbeatSignalType.ABSENCE, new Date('2024-01-01T05:00:00Z')),
        makeEntry(HeartbeatSignalType.ABSENCE, new Date('2024-01-01T10:00:00Z')),
      ];
      const info = AnalyticsEngine.computeStreakInfo(entries, now);
    ...
    it('should compute timeSinceLastPresenceMs correctly', () => {
      const entries = [
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T12:00:00Z')),
        makeEntry(HeartbeatSignalType.ABSENCE, new Date('2024-01-01T18:00:00Z')),
      ];
      const info = AnalyticsEngine.computeStreakInfo(entries, now);
  ...
  describe('formatCSV', () => {
    ...
    it('should produce header + 1 row per entry', () => {
      const entries = [
        makeEntry(HeartbeatSignalType.PRESENCE, new Date('2024-01-01T00:00:00Z')),
        makeEntry(HeartbeatSignalType.ABSENCE, new Date('2024-01-01T01:00:00Z')),
      ];
    ...
    it('should quote fields containing commas', () => {
      const entries = [
        makeEntry(HeartbeatSignalType.CHECK_FAILED, new Date('2024-01-01T00:00:00Z'), {
  ...
  describe('edge cases', () => {
    ...
    it('computeStatistics: all entries are CHECK_FAILED', () => {
      ...
      const until = new Date('2024-01-02T00:00:00Z');
      const entries = [
        makeEntry(HeartbeatSignalType.CHECK_FAILED, new Date('2024-01-01T01:00:00Z')),
        makeEntry(HeartbeatSignalType.CHECK_FAILED, new Date('2024-01-01T02:00:00Z')),
        makeEntry(HeartbeatSignalType.CHECK_FAILED, new Date('2024-01-01T03:00:00Z')),
      ];
      const stats = AnalyticsEngine.computeStatistics(entries, since, until);
