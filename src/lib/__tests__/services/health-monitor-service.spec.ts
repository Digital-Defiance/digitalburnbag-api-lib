...
class MockConnectionRepo implements IConnectionRepository<string> {
  ...
  public updates: Array<{ id: string; updates: Partial<IProviderConnectionExtended<string>> }> = [];

  addConnection(conn: IProviderConnectionExtended<string>): void {
    this.connections.set(conn.id, conn);
  }

  async getConnection(connectionId: string): Promise<IProviderConnectionExtended<string> | null> {
...
class MockCredentialService implements ICredentialService<string> {
  ...
  public freshness: ICredentialFreshnessResult = { valid: true, expiresInMs: 3_600_000 };
  public shouldThrowOnRefresh = false;

  setCredentials(connectionId: string, creds: IProviderCredentials<string>): void {
    this.credentials.set(connectionId, creds);
  }

  async storeCredentials(credentials: IProviderCredentials<string>): Promise<void> {
    const key = String(credentials.userId) + ':' + String(credentials.providerId);
    this.credentials.set(key, credentials);
  }

  async getDecryptedCredentials(connectionId: string): Promise<IProviderCredentials<string>> {
...
function buildService(opts: {
  ...
  return { service, connRepo, historyRepo, credService, failureManager, registry };
}
...
describe('HealthMonitorService', () => {
  ...
  describe('token refresh failure (Req 6.6)', () => {
    it('marks connection as "expired" and notifies user when refresh fails', async () => {
      ...
      const refreshed = await service.refreshTokensIfNeeded('conn-refresh-fail');
  ...
  describe('retry during feed failure (Req 3.6)', () => {
    it('continues executing checks even after CHECK_FAILED results', async () => {
      ...
      (adapter.checkHeartbeat as jest.Mock)
        .mockImplementation(() => {
          callCount++;
          if (callCount === 1) {
