...
describe('Feature: canary-provider-system, Property 16: Provider registry registration and retrieval', () => {
  ...
  it('built-in configs are retrievable after initialization', () => {
    ...
    fc.assert(
      fc.property(
        fc.array(arbValidConfig, { minLength: 1, maxLength: 10 }).map((configs) => {
          // Ensure unique IDs
        ...
        (configs) => {
          const registry = new CanaryProviderRegistry<string>(configs, validator);
    ...
    fc.assert(
      fc.property(arbValidConfig, (config) => {
        ...
        return (
          ...
          reimportedConfig.endpoints.activity.path === config.endpoints.activity.path &&
