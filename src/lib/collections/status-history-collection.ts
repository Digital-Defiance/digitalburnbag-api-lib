  ...
  async getEntriesByConnection(
    ...
    if (options?.since) {
      ...
      entries = entries.filter((e) => {
        const ts = e.timestamp instanceof Date ? e.timestamp : new Date(e.timestamp as unknown as string);
    ...
    entries.sort((a, b) => {
      const aTime = a.timestamp instanceof Date ? a.timestamp.getTime() : new Date(a.timestamp as unknown as string).getTime();
      const bTime = b.timestamp instanceof Date ? b.timestamp.getTime() : new Date(b.timestamp as unknown as string).getTime();
      return aTime - bTime;
    });
