...
export interface IConnectionRepository<TID extends PlatformID = string> {
  getConnection(connectionId: TID): Promise<IProviderConnectionExtended<TID> | null>;
  updateConnection(
    connectionId: TID,
  ...
  implements IHealthMonitorService<TID>
{
  ...
  async executeCheck(connectionId: TID): Promise<IHeartbeatCheckResult<TID>> {
    ...
    try {
      credentials = await this.credentialService.getDecryptedCredentials(connectionId);
    } catch {
      // Credential retrieval failed — treat as CHECK_FAILED
      const failResult = this.buildFailedResult(connectionId, 'Credential retrieval failed');
      await this.processResult(connection, failResult, HeartbeatSignalType.CHECK_FAILED);
      return failResult;
    }
    ...
    const adapter = this.registry.getProvider(connection.providerId);
    if (!adapter) {
      const failResult = this.buildFailedResult(connectionId, 'Provider adapter not found');
      await this.processResult(connection, failResult, HeartbeatSignalType.CHECK_FAILED);
      return failResult;
    }
    ...
    try {
      const since = new Date(Date.now() - (adapter.getConfig().defaultLookbackMs ?? 86_400_000));
      ...
      result = this.buildFailedResult(connectionId, 'Heartbeat check threw an exception');
    }
  ...
  async refreshTokensIfNeeded(connectionId: TID): Promise<boolean> {
    ...
    const freshness = await this.credentialService.validateCredentialFreshness(connectionId);
    ...
    try {
      const credentials = await this.credentialService.getDecryptedCredentials(connectionId);
